...
/// <summary>
/// End-to-end acceptance for the Postgres provider: real Postgres (Testcontainers) +
/// in-process SignalR hub wired via <c>AddPostgresSource</c> + real client connection.
/// Mirrors the Mongo end-to-end suite: filtered snapshot first, then live changes, then
/// restart-resume through the file-backed resume token store.
/// </summary>
public sealed class PostgresEndToEndTests : IClassFixture<PostgresContainerFixture>, IAsyncLifetime
{
    private readonly PostgresContainerFixture _fixture;
    private NpgsqlConnection _conn = null!;
    private string _table = null!;

    public PostgresEndToEndTests(PostgresContainerFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task InitializeAsync()
    {
        ...
        cmd.CommandText = $"""CREATE TABLE "{_table}" (id text PRIMARY KEY, status text NOT NULL, total bigint)""";
    ...
    {
        var resumeDir = Path.Combine(Path.GetTempPath(), "pulse_pg_resume_it_" + Guid.NewGuid().ToString("N"));
        try
        {
